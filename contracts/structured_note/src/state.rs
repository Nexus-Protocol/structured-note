...
pub fn decrease_withdraw_amount(storage: &mut dyn Storage, amount: Uint128) -> StdResult<Uint128> {
    KEY_WITHDRAW_AMOUNT.update(storage, |mut value| -> StdResult<Uint128> {
        value -= amount;
        Ok(value)
    })
}
