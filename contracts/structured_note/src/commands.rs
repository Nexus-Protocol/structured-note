...
pub fn withdraw_stable(deps: DepsMut, state: State, amount: Uint128) -> StdResult<Response> {
    let cdp_idx = if let Some(i) = state.cdp_idx {
        i
    } else {
        return Err(StdError::generic_err("cdp_idx has to be stored by now"));
    };
    //considering that user passed mAsset amount to withdraw from farming to obtain
    let amount_to_withdraw = amount * state.asset_price_in_collateral_asset;
    withdraw_collateral(&config, cdp_idx, amount_to_withdraw)
}